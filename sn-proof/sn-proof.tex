...
%  \item\label{pp3} {\color{red}{If $\Gamma \vdash [N/x]M : B \in \csn$ and $\Gamma \vdash N : A \in
...
\end{lemma}
\begin{proof}
Property \ref{pp2} is proven by mutual induction on the derivations; properties \ref{pp6}, \ref{pp7}, and \ref{pp4} are proven by induction on the first derivation. 
\\[1em]
...
$\Gamma \vdash R : B \in \csn$ \hfill by Lemma \ref{lem:psn}(\ref{pp2}) since $C[x]$ is not a lambda-abstraction\\
...
\begin{lemma}\label{lem:SNsn}
If   $\Gamma \vdash M \redSN M_1 : A$ and $\Gamma \vdash M_1 : A \in \SN$ and 
$\Gamma \vdash M_1 : A \in \csn$ and $\Gamma \vdash M \red M_2$
then $\Gamma \vdash M_2 : A \in \csn$.  
\end{lemma}
\begin{proof}
Proof by case analysis and induction; we may need to generalize the
statement to use evaluation context.

\paragraph{Case} $\ianc{\Gamma \vdash N : A \in \SN \quad \Gamma, x{:}A \vdash M : B}
                       {\Gamma \vdash (\lambda x.M)\;N \redSN [N/x]M : B}{}$ 
            \qquad and \qquad$\ianc{\Gamma \vdash \lambda x{:}A.M : A \arrow B \quad \Gamma \vdash  N : A}
                       {\Gamma \vdash (\lambda x.M)\;N \red [N/x]M : B}{}$
\\[1em]
$\Gamma \vdash [N/x]M:B \in \csn$ \hfill by assumption (and $M_1 = M_2$)

\paragraph{Case} $\ianc{\Gamma \vdash N : A \in \SN \quad \Gamma, x{:}A \vdash M : B}
                       {\Gamma \vdash (\lambda x.M)\;N \redSN [N/x]M : B}{}$
\qquad and \qquad$\ibnc{\Gamma \vdash \lambda x.M \red \lambda x.M' : A \arrow B}
                        {\Gamma \vdash N : A}
                       {\Gamma \vdash (\lambda x.M)~N \red (\lambda x.M')~N : B}{}$
\\[1em]
$\Gamma, x:A \vdash M \red M' : B$ \hfill by inversion on $\red$\\
$\Gamma \vdash [N/x]M : B \in \SN$ \hfill by assumption \\
$\Gamma \vdash [N/x]M : B \in \csn$ \hfill by assumption \\
{\color{red}{TO SHOW: $\Gamma \vdash (\lambda x.M')~N : B \in \csn$}}

\paragraph{Case} $\ianc{\Gamma \vdash N : A \in \SN \quad \Gamma, x{:}A \vdash M : B}
                       {\Gamma \vdash (\lambda x.M)\;N \redSN [N/x]M : B}{}$
\qquad and \qquad$\ianc{\Gamma \vdash \lambda x.M : A \arrow B \quad \Gamma \vdash N \red N' : A}
                       {\Gamma \vdash (\lambda x.M)~N \red (\lambda x.M)~N' : B}{}$
\\[1em]
$\Gamma \vdash [N/x]M : B \in \SN$ \hfill by assumption \\
$\Gamma \vdash [N/x]M : B \in \csn$ \hfill by assumption \\
...
\quad and \quad  $\ianc{\Gamma \vdash R \red M'' : A \arrow B \quad \Gamma \vdash N : A}
                      {\Gamma \vdash R~N \red M''~N: B}{}$\\[0.5em]
                      where $R$ is not a $\lambda$-abstraction
\\[1em]
% {\color{red}{TO SHOW: $\Gamma \vdash M''~N:B \in \csn$}} \\
$\Gamma \vdash M'\,N : B \in \SN$ \hfill by assumption \\
$\Gamma \vdash M' : A \rightarrow B \in \SN$ \hfill by Lemma \ref{lem:SNApp} \\
$\Gamma \vdash M'\,N : B \in \csn$ \hfill by assumption \\
$\Gamma \vdash M' : A \rightarrow B \in \csn$ \hfill by Property \ref{lem:psn}(\ref{pp6}) \\
$\Gamma \vdash R \redSN M' : A \rightarrow B$ and $\Gamma \vdash R \red M'' : A \rightarrow B$ \hfill by assumption \\
$\Gamma \vdash M'' : A \rightarrow B \in \csn$ \hfill by IH \\
$\Gamma \vdash N : A \in \csn$ \hfill by Property \ref{lem:psn}(\ref{pp6}) \\
$\Gamma \vdash M''~N : B \in \csn$ \hfill by Property \ref{lem:psn}(\ref{pp2})

\paragraph{Case} $\ianc{\Gamma \vdash R \redSN M' : A \arrow B \quad \Gamma \vdash N : A}{\Gamma \vdash R\,N \redSN M'\,N}{}$
\quad and \quad $\ianc{\Gamma \vdash N \red N' : A \arrow B \quad \Gamma \vdash R : A \arrow B}
                      {\Gamma \vdash R~N \red R~N' : B}{}$ 
\\[0.5em] where $R$ is not a $\lambda$-abstraction
\\[1em]
{\color{red}{TO SHOW: $\Gamma \vdash R~N':B \in \csn$}}
\end{proof}
...
{\color{red}{$\Gamma \vdash M : A \in \csn$ \hfill by Lemma \ref{lem:SNsn}}}
...
%\\[1em]
%\noindent
% 3.  If $\Gamma \vdash M \redSN M' : A$
%     then $\Gamma \vdash M \redsn M' : A$.
