...
\end{lemma}
\begin{proof}
Proof by case analysis and induction on evaluation context ?.
...
{\color{red}{TO SHOW: $\Gamma \vdash (\lambda x.M)~N' : B \in \csn$}}
%
\paragraph{Case} $\ianc{\Gamma \vdash R \redSN R' : A \arrow B \quad \Gamma \vdash N : A}{\Gamma \vdash R\,N \redSN R'\,N}{}$
\quad and \quad  $\ianc{\Gamma \vdash R \red R'' : A \arrow B \quad \Gamma \vdash N : A}
                      {\Gamma \vdash R~N \red R''~N: B}{}$\\[0.5em] where $R$ is not a $\lambda$-abstraction
\\[1em]
