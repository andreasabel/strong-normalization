...
The typed reduction strategy satisfies weakening and strengthening.

\begin{lemma*}[Weakening and Strengthening of Reductions]\label{lem:redprop}\quad
  \begin{itemize}
...
\infer{\Gamma \vdash M : A \in \csn}
      {\forall M'.~\Gamma \vdash M \red M' : A \imply \Gamma \vdash M' : A \in \csn}
\]
\end{definition}
...
$\Gamma \vdash N : A \in \csn$,~ and $B$. We silently exploit type uniqueness. \\[1em]
...
\paragraph{Sub-case:} $M = x$
\\
$\Gamma \vdash [N/x]x \in \csn$ \hfill by assumption $\Gamma \vdash N \in \csn$\ednote{There should be another case for $M = y$ different from $x$ -am}
%
\paragraph{Sub-case:} $M = M_1~M_2$
\\
...
Using evaluation contexts we can now state that strongly normalizing terms are closed under expansion.
...
%    \vdash M \in \csn$.
  ...
  \item\label{cp3b} If $\Gamma \vdash C[x]~M \red R$ and $\Gamma \vdash C[x] \in
