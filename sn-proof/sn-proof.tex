...
\begin{metanote}
	Properties (\ref{cp3}) and (\ref{cp3b}) follow from Lemma \ref{lem:psn} (\ref{pp2}) as $C[x]$ corresponds to the terms that \ref{lem:psn} (\ref{pp2}) applies to, and (\ref{cp2}) is never actually referenced, but instead the same one-step proof is repeated whenever we need to establish that a variable is $\csn$. Do we really need these three properties? -ah
\end{metanote}
