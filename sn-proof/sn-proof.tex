...
\[
\begin{array}{llcl}
\mbox{Terms}  & M,N & \bnfas & x \mid \lambda x. M \mid M\;N \\
\end{array}
\]
...
(\lambda x.M)\;N & \red & [N/x]M \qquad\qquad\mbox{$\beta$-reduction}
\end{array}
\]
...
\[
\begin{array}{l}
\infer{M\,N \red M'\,N}{M \red M'} \qquad  \infer{M\,N \red M\,N'}{N \red N'}
\qquad
\infer{\lambda x.M \red \lambda x.M'}{M \red M'}
\end{array}
\]
...
We can define the meaning of a well-typed term $M$ in the context $\Gamma$ of type $A$ as follows: for all grounding instantiations $\sigma$ providing values for the variables declared in $\Gamma$, $[\sigma]M$ is in the denotation of $A$. We write for the denotation of $A$ as $\den{A} = \A$. Similarly, the denotation of $\Gamma$ is written as $\den{\Gamma}=\G$.

$\den{A}$ is interpreted as the sets of strongly normalizing terms of type $A$, i.e. $\den{A} \in \SN$. We prove that if a term is well-typed, then it is strongly normalizing in  two steps:
...
\\[1em]
We remark first, that all variables are in the denotations of a type $A$, i.e. $\textsf{Var} \subseteq \den{A}$, and variables are strongly normalizing, i.e. they are already in normal form.
\\[1em]
Next, we define the denotations of the base type $o$ and the function type $A \arrow B$.
% Recall, we consider here the simply typed lambda calculus with base type $o$.

% \[
% \begin{array}{llcl}
% \mbox{Types} & A,B,C & \bnfas & o \mid A \arrow B  
% \end{array}
% \]

% We need to give a type interpretation (denotation) for each of the types.

\begin{itemize}
\item A term $M \in \den{o}$ iff $M\hastype o$ and $M$ is strongly normalizing, i.e. $M \in \SN$.
\item A term $M \in \den{A \arrow B}$ iff $M \in \den{A} \imply \den{B}$,
  i.e. for all $N \in \den{A}. M\;N \in \den{B}$.
\end{itemize}
...
\mbox{Semantic function type} & \den{A \arrow B} & := & \{ M | \forall N \in \den{A}. M\;N \in \den{B} \}
\end{array}
\]
...
normalizing.\\
\[
\infer{\csn~ M}{\forall M'.~M \red M' \imply \csn~ M'}
\]
\end{definition}

Moreover, we have that if a given term $M$ is strongly normalizing, then any subterm must be strongly normalizing as well. We omit the proof here and leave it to an exercise\ednote{I don't have a good structural def of subterm, so not an easy exercise --am}. 
...
\begin{center}
\begin{tabular}{ll}
$M \in \SN$  & $M$ is in the set of normal terms\\
$M \in \SNe$ & $M$ is in the set of neutral terms 
\end{tabular}  
\end{center}
...
\ianc{}{x\in \SNe}{} \qquad   \ibnc{R \in \SNe}{M \in \SN}{R\,M \in \SNe}{} 
\\[1em]
\multicolumn{1}{l}{\mbox{Normal terms}} \\[1em]
\ianc{R \in \SNe}{R \in \SN}{} \qquad 
\ianc{M \in \SN}{\lambda x.M \in \SN}{} \qquad
\ibnc{M \redSN M'}{M' \in \SN}{M \in \SN}{} 
\\[1em]
\multicolumn{1}{l}{\mbox{Strong head reduction}} \\[1em]
\ianc{N \in \SN}{(\lambda x.M)\;N \redSN [N/x]M}{} \qquad
\ibnc{R \redSN R'}{R\;\text{is not a}\;\lambda}{R\,M \redSN R'\,M}{}
\end{array}
\]
...
We will sketch here that these inductive definition is sound and complete with respect to our informal understanding of strongly normalizing reductions (Def. \ref{def:norm}). 
...
\begin{lemma}[Properties of strongly normalizing terms]$\;$
  \begin{enumerate}
  \item\label{pp1} If $M \in \csn$ and $[N/x]M \in \csn$ and $N \in \csn$ 
        then $(\lambda x.M) \;N \in \csn$.\ednote{by first part of closure lemma, $M \in \csn$ non needed }
  \item\label{pp2} If $M \in \csn$ and $N \in \csn$ where $M$ is not a $\lambda$
        then $M\;N \in \csn$. (we also have $M\;N \red M'\;N$ and $M'\;N \in \csn$ as i.h.)
  \end{enumerate} 
\end{lemma}
\begin{proof}
By induction on $M \in \csn$ and $N \in \csn$.  
\end{proof}

\begin{lemma}[Closure properties of strongly normalizing terms]$\;$
  \begin{enumerate}
  \item\label{cp1} If $[N/x]M \in \csn$ then $M \in \csn$.
  \item\label{cp2} For all variables $x$, $x \in \csn$.
  \item\label{c31} If $M \in \csn$ and $N \in \csn$ where $M = x \,\vec N$ 
     then $M\,N \in \csn$.
   \item\label{cp4} If $M \in \csn$ then $\lambda x.M \in \csn$.
   \item\label{cp5} {$\csn$-backward closure:} If $M \redsn M'$ and $M' \in \csn$ then
     $M \in \csn$ where
\[
\begin{array}{l}
\infer{(\lambda x.M)\;N \redsn [N/x]M}{N \in \csn} 
\qquad
\infer{M\;N \redsn M'\;N}{M \redsn M' & M\,\text{is not a}\;\lambda}  %& s \in \csn
  \\[1em]
\end{array}
\]
\end{enumerate}
...
\mbox{}
  \begin{enumerate}
  \item If $M \in SN$ then $M \in \csn$.
  \item If $M \in SNe$ then $M \in \csn$ and $M = x\, \vec N$.
  \item If $M \redSN M'$ then $M \redsn M'$.
  \end{enumerate} 
\end{theorem}
\begin{proof}
By mutual structural induction on the given derivations using the closure properties. 
\begin{enumerate}
\item $\xi$-case uses Property~\ref{cp4}. REduction case uses~\ref{cp5}
\item Variable case uses~\ref{cp2}, application uses~\ref{pp2}
\item Follows by approriate calls to induction hypotheses
\end{enumerate}
\end{proof}
