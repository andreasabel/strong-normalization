...
While this definition is very concise, we can show that it enjoys the expected closure and substitution properties, namely:
\begin{itemize}
\item $\Gamma \vdash M \in \csn$ and $\Gamma \vdash N \in \csn$
    iff $\Gamma \vdash M~N \in \csn$;
  \item $\Gamma, x{:}A \vdash M \in \csn$ iff $\Gamma \vdash  \lambda x.M \in \csn$;
  \item Let  $\Gamma \vdash N \in
   \csn$. Then $\Gamma, x{:}A \vdash M \in \csn$ iff $\Gamma \vdash [N/x]M \in \csn$.
\end{itemize}
...
$\Gamma, x{:}A \vdash x \in \csn$ \hfill trivial since there is no reduction $x \red x$\ednote{There should be another case for $M = y$ different from $x$ -am}

\paragraph{Sub-case:} $M = M_1~M_2$ \\
$[N/x](M_1~M_2) = [N/x]M_1~[N/x]M_2$ \hfill by def. of subst. \\
$\Gamma \vdash [N/x]M_1 \in \csn$ and $\Gamma \vdash [N/x]M_2 \in \csn$ \hfill by Property \ref{pp6} \\
...
\begin{metanote}
  We may want to explain the notation $C[x]$, as the inductin
...
Here, we define inductively the set of normal terms, $\SN$, and the set of neutral terms, $\SNe$,   using the following judgments:
\\%[1em]
