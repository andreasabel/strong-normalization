...
We now list some standard properties of (typed) reduction. We start
with the fact that typed reductions are only defined on well-typed
terms, i.e. if $M$ steps then $M$ is well-typed. The rules for typing
...
$\C : \Gamma \vdash P : B \in \csn$ and $\C \leq \D$ \hfill using $\D : \Gamma \vdash M'~N : B \in \csn$ and Lemma \ref{lm:mredsn}
\\
$\C < D$ \hfill since we took at least one step using  $\Gamma \vdash M'~N \mred P : B$\\
