...
From this definition, it easily follows:
 ...
 If $\Gamma \vdash M \mred M' : B$ and $\Gamma \vdash M : B \in \csn$ then $\Gamma \vdash M' : B \in \csn$.
 \end{lemma}
...
\begin{lemma}[Anti-renaming (Strengthening) of strongly normalizing terms]
...
\begin{lemma}[$\SNe$ is closed under application]\label{lm:pSN0}
...
$\Gamma \vdash C[~[N/x]M~] : B \in \csn$  is getting smaller\ednote{In the Agda implementation, this lemma uses $\Gamma \vdash C[~[N/x]M~] : B \in \csn$ which means it cannot be proven independently; this does not make sense to me. Otherwise the proof follows the same main ideas. -bp}.
\\[1em]
\noindent
