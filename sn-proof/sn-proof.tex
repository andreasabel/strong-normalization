...
The inductive definition given in Fig.~\ref{fig:sn}.
...
\begin{lemma}[Stable under Substitution]\label{lem:SNsubst}\quad
  \begin{enumerate}
  \item If $\Gamma, x{:}A \vdash M \in \SN$ and $\Gamma \vdash N \in \SN$ 
    then $\Gamma \vdash [N/x]M \in \SN$.
  \item If $\Gamma, x{:}A \vdash R \in \SNe$ and $\Gamma \vdash N \in \SN$ 
    then $\Gamma \vdash [N/x]R \in \SN$.
  \item If $\Gamma, x{:}A \vdash M \redSN M'$ and $\Gamma \vdash N \in \SN$ then 
    $\Gamma \vdash [N/x]M \redSN [N/x]M'$. 
  \end{enumerate}
\end{lemma}
\begin{proof}
...
If $\Gamma \vdash M \in \SN$ and $\Gamma \vdash N \in \SN$ then $\Gamma \vdash M~N \in \SN$.    
\end{lemma}
\begin{proof}
By induction on $\SN$.
...
\paragraph{Case:} $\D = \ianc{\Gamma \vdash R \in \SNe}{\Gamma \vdash R \in \SN}{}$
\\[1em]  
$\Gamma \vdash R~N \in \SNe$ \hfill by def. of $\SNe$\\
$\Gamma \vdash R~N \in \SN$ \hfill by def. of $\SN$

\paragraph{Case} $\D = \ianc{\Gamma, y{:}A \vdash M \in \SN}{\Gamma \vdash \lambda y{:}A.M \in \SN}{}$
\\[1em]
$\Gamma \vdash [N/y]M \in \SN$ \hfill by substitution lemma \ref{lem:SNsubst}\\
$\Gamma \vdash N \in \SN$ \hfill by assumption \\
$\Gamma \vdash (\lambda y{:}A.M)~N \redSN [N/y]M$ \hfill by def. of $\redSN$\\
$\Gamma \vdash (\lambda y{:}A.M)~N \in \SN$ \hfill by def. of $\SN$

\paragraph{Case} $\D = \ibnc{\Gamma \vdash M \redSN M'}{\Gamma \vdash M' \in \SN}
                            {\Gamma \vdash M \in \SN}{}$\\[1em]
$\Gamma \vdash M~N \redSN M'~N$ \hfill by def. of $\redSN$\\
$\Gamma \vdash M'~N \in \SN$ \hfill by IH \\
$\Gamma \vdash M~N \in \SN$ \hfil by def. of $\SN$.
\end{proof}

\begin{lemma}[$\SN$ is closed under application to variables]\label{lm:pSN1}
If $\Gamma \vdash M \in \SN$ and $x{:}A \in \Gamma$ then \mbox{$\Gamma \vdash M~x \in \SN$}.  
\end{lemma}
\begin{proof}
Follows from the previous lemma \ref{lm:pSN}.
...
If  $x{:}A \in \Gamma$ and $\Gamma \vdash M~x \in \SN$ then $\Gamma \vdash M \in \SN$.
\end{lemma}
\begin{proof}
By induction on $\SN$
