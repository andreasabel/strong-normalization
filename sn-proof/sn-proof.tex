...
\paragraph{Case} $\D = \Gamma \vdash C[~(\lambda x.M)~N~] \red C'[~(\lambda x.M)~N~]$, i.e. we reduce, but not in a head position; if $C = \underline{\quad}~N_0\ldots N_n$ and there is a reduction $\Gamma \vdash N_i \red N'_i : A_i$. Furthermore $C' = \underline{\quad}~N_0\ldots N'_i \ldots N_n$.
\\[0.5em]
...
% \paragraph{Case}
% $\D = \ianc{C'[(\lambda x.M)~N] \red Q : B}{\Gamma \vdash C[C'[(\lambda x.M)~N]] \red C[Q] : B'}{}$ -- is this case actually necessary?
% \\
% $\Gamma, x{:}B \vdash C'[x] ~N_0 : B' \in \csn$ \hfill by assumption 
% $\Gamma \vdash C'[\,(\lambda x.M)~N\,] \mred C'[~[N/x]M~] : A' \arrow B'$ \hfill \\
% $\Gamma \vdash C'[\,(\lambda x.M)~N\,]~N_0 \mred C'[~[N/x]M~]~N_0 : B'$ \hfill \\
% $\Gamma \vdash C'[~[N/x]M~]~N_0 : B \in \csn$ \hfill by assumption \\
%
% 
% $\Gamma \vdash C'[\,(\lambda x.M)~N\,]~N_0 : B' \in \csn$ \hfill by IH
