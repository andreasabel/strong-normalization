...
\begin{itemize}
\item A term $M \in \den{o}$ iff $M$ is strongly normalizing, i.e. $M \in \SN$.
...
\ianc{}{x\in \SNe}{} \qquad   \ibnc{R \in \SNe}{s \in \SN}{R\,M \in \SNe}{} 
\\[1em]
...
\begin{lemma}[Properties of strongly normalizing terms]$\;$
  \begin{enumerate}
  \item If $M \in \csn$ and $[N/x]M \in \csn$ and $N \in \csn$ 
        then $(\lambda x.M) \;N \in \csn$.
  \item If $M \in \csn$ and $N \in \csn$ where $M$ is not a $\lambda$
...
\begin{lemma}[Closure properties of strongly normalizing terms]$\;$
  \begin{enumerate}
  \item If $[N/x]M \in \csn$ then $M \in \csn$.
  \item For all variables $x$, $x \in \csn$.
  \item If $M \in \csn$ and $N \in \csn$ where $M = x \,\vec N$ 
     then $M\,N \in \csn$.
   \item If $M \in \csn$ then $\lambda x.M \in \csn$.
   \item \textbf{Expansion.} If $M \redsn M'$ and $M' \in \csn$ then $M \in \csn$
where   
\[
\begin{array}{l}
\infer{(\lambda x.M)\;N \redsn [N/x]t}{N \in \csn} 
\qquad
...
\end{theorem}
\begin{proof}
By mutual structural induction on the given derivation using the closure properties.   
\end{proof}

\begin{theorem}[Completeness of $\SN$]
  \begin{enumerate}
